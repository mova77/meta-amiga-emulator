...
| **Date** | 2026-09-23 |
| **Produced by** | MAE-9 (spike), the paper half of its time-box |
...
AGA clause is unmet and `MAE-11` must be written for OCS/ECS only. This is a scope finding,
...
SPIKE-S0 §4 and `MAE-12`, not this table.
...
7. **Copper slot cost.** Figure 6-9's footnote states *"Copper Data Move instruction
   ...
   SPIKE-S0 §4 and `MAE-12`, not slot allocation — recorded here so it is not lost, and
...
### 9.1 Deliverables MAE-9 asked for that are not here
...
- **`src/core/chipset/slot_tables.hpp`.** The story asks for a table file the build
  ...
  change to the core library, which this piece of work does not touch. `MAE-11` generates
